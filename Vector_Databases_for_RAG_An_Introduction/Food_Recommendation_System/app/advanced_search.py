...
def populate_similarity_collection(collection, food_items):
    """Populate a collection with food items across common vector-store APIs."""
    if not collection:
    ...
    for index, item in enumerate(food_items):
        docs.append(item.get("food_description") or item.get("description") or "")
        metadata = {
            key: value
            for key, value in item.items()
            if key not in {"food_description", "description"}
        }
        metas.append(metadata)
        ids.append(str(index))

    for method_name in ("add", "add_documents", "insert", "upsert"):
        method = getattr(collection, method_name, None)
        if callable(method):
            try:
                return method(documents=docs, metadatas=metas, ids=ids)
            except TypeError:
                try:
                    return method(docs, metas, ids)
                except TypeError:
                    try:
                        return method(food_items)
                    except TypeError:
                        continue

    raise AttributeError(
        "The collection object does not expose a supported document-population method."
    )
