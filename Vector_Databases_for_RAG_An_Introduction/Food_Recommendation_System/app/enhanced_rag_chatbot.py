...
from advanced_search import populate_similarity_collection
...
def main():
    ...
    except (FileNotFoundError, KeyError, ValueError, ConnectionError) as error:
        print(f"❌ Error: {error}")
...
Response:'''
    ...
    except (ConnectionError, KeyError, IndexError, TypeError, ValueError) as e:
        print(f"❌ LLM Error: {e}")
        return generate_fallback_response(query, search_results)
...
def enhanced_rag_food_chatbot(collection):
    ...
    while True:
        try:
            ...
            break
        except (ConnectionError, KeyError, TypeError, ValueError, RuntimeError) as error:
            print(f"❌ Bot: Sorry, I encountered an error: {error}")
...
Comparison:'''
    ...
    except (KeyError, IndexError, TypeError, ValueError, ConnectionError):
        return generate_simple_comparison(query1, query2, results1, results2)
