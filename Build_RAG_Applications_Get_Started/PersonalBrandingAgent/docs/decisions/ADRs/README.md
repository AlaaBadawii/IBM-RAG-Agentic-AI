...
| ADR-008 | Cron triggers the workflows; no Python scheduler, no systemd timer | Accepted (see `../../../ops/personal-branding-agent.cron`, `../../operations/scheduling.md`) |
| ADR-009 | Stale locks recover on liveness, never on timeout alone | Accepted (see `../../../app/workflows/scheduled.py`) |
| ADR-010 | Workflow-level evaluation with fakes over real boundaries | Accepted (see `../../../tests/test_autonomous_evaluation.py`, `../../evaluation/autonomous-evaluation.md`) |
